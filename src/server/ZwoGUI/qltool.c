...
static u_long     black,green,yellow,white,red,l_off=0;
                      ...
                      int x0,int y0,int dimx,int dimy,int baseI)
{
  ...
  CBX_Lock(0);
  if (!l_off) {
    black = app->black; green = app->green;
    { XColor xcol,ycol;
  ...
  qlt->lmag = 2;                       /* default magnification */
  ...
  w = 15*PXw;
  x = gWIDE - w - PXw/2;
  for (i=0; i<QLT_NCURSORS; i++) {     /* Note: 3+XXh in gui */
    y += 3+XXh+PXh/3;                  /* below "cursorStep" */
    CBX_CreateAutoOutput_Ext(mw,&qlt->cubox[i],parent,x,y,w,XXh," 0 0 0");
  }

  qlt->ix = x0-3 + (gWIDE-qlt->iWIDE)/2;
  qlt->iy = imax(3+y0+qlt->lHIGH,1+y0+6*(1+XXh+PXh/3))+2*(0+2)+2;
  CBX_Lock(0);
...
#if 0 /* Shec: no green center pixel v0054 */
...
static void create_image(QlTool* qlt,int min,int dyn,void* p0)
{
  ...
  for (i=0; i<QLT_NCURSORS; i++) {     /* overlay cursors */
    color = (i == qlt->cursor_mode) ? green : black;
